...
from models.market import MarketTransactionModel
from schemas.market import MarketTransaction


def get_transaction(ship_symbol: str, timestamp: datetime):
    with Session(engine) as session:
        return _transaction_to_schema(_get_transaction(ship_symbol, timestamp, session))


def get_create_transaction(trans: MarketTransaction)-> MarketTransaction:
    with Session(engine) as session:
       return  _transaction_to_schema(_get_create_transaction(trans, session))


def _get_create_transaction(trans: MarketTransaction, session: Session) -> MarketTransactionModel:
...
def _transaction_to_schema(trans: MarketTransactionModel) -> MarketTransaction:
    if not trans:
        return None
